...
class BaseNode:
    id: str
    position: Point
    props: Dict[str, Any] = field(default_factory=dict)  # editable props
    state: Dict[str, Any] = field(default_factory=dict)  # computed state
    status: str = "online"  # online/offline (node)


@dataclass
class Source(BaseNode):
    kind: str = "plant"  # HV source
...
class Line:
    ...
    # props: capacity_kva (или capacity_kw), length, voltage_kv, r_pu, x_pu, switchable(bool)
    state: Dict[str, Any] = field(default_factory=dict)
    # state: p_flow_kw, q_flow_kvar, s_flow_kva, loss_kw, overload_ratio, island_id
